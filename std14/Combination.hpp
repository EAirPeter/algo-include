...
// Generate modular multiplicateive inverse table of factorials for all
// 0 <= i <= N, modulo Mod
// Require: FacN = Fac(N), Gcd(FacN, Mod) = 1
template<class Unsigned, class ModRes = Unsigned>
constexpr void GenFacInvTable(Unsigned FacInv[], Unsigned N,
  Unsigned FacN, Unsigned Mod) noexcept
{
  using MA = ModArith<Unsigned, ModRes>;
...
// Return: C(M, N)
template<class Unsigned>
constexpr Unsigned Comb(Unsigned M, Unsigned N, const Unsigned Fac[])
  noexcept
{
...
// Require: 0 <= N <= M; N < Mod
// Require: factorial table Fac, multiplicative inverse table of factorials
//          FacInv
// Return: C(M, N) % Mod
template<class Unsigned, class MulRes = Unsigned>
constexpr Unsigned Comb(Unsigned M, Unsigned N, Unsigned Mod,
  const Unsigned Fac[], const Unsigned FacInv[]) noexcept
{
  using MA = ModArith<Unsigned, MulRes>;
...
// Return: C(M, N) % Mod
template<class Unsigned, class MulRes = Unsigned>
constexpr Unsigned CombLucas(Unsigned M, Unsigned N, Unsigned Mod,
  const Unsigned Fac[], const Unsigned FacInv[]) noexcept
{
  using MA = ModArith<Unsigned, MulRes>;
  Unsigned res = 1;
  while (N >= Mod) {
    res = MA::Mul(res, Comb<Unsigned, MulRes>(M % Mod, N % Mod,
      Mod, Fac, FacInv), Mod);
    M /= Mod;
    N /= Mod;
  }
...
// Return: {rem, exp}
template<class Unsigned, class MulRes = Unsigned>
constexpr FacExRes<Unsigned> FacEx(Unsigned N, Unsigned Mod, Unsigned P)
  noexcept
{
  using MA = ModArith<Unsigned, MulRes>;
...
// Require: P must be prime, Mod must be power of P
template<class Unsigned, class MulRes = Unsigned>
constexpr Unsigned CombLucasEx(Unsigned M, Unsigned N, Unsigned Mod,
  Unsigned P) noexcept
{
  using MA = ModArith<Unsigned, MulRes>;
  ...
  return MA::Mul(MA::Div(A.rem, MA::Mul(B.rem, C.rem, Mod), Mod),
    MA::Pow(P, A.exp - B.exp - C.exp, Mod), Mod);
}
