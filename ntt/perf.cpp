#include "Ntt.hpp"
#define NTT_USE_AVX2
#include "Ntt.hpp"

#include <algorithm>
#include <chrono>
#include <cstdint>
#include <cstring>
#include <functional>
#include <iostream>
#include <limits>
#include <random>

#ifdef _MSC_VER
#define restrict_ __restrict
#define MFence(...) _ReadWriteBarrier()
#else
#define restrict_ __restrict__
#define MFence(...) __sync_synchronize()
#endif

using namespace std;
using namespace chrono;

constexpr int LogLen = 20;
constexpr size_t Len = size_t{1} << LogLen;
constexpr size_t Len2 = Len >> 1;
constexpr uint32_t Mod = 998244353;
constexpr uint32_t G = 3;

namespace csp5 {

using I32 = ::std::int32_t;
using U32 = ::std::uint32_t;
using U64 = ::std::uint64_t;

constexpr U32 GInv = 332748118;
constexpr U32 BmM = 288737297;
constexpr U32 BmW = 29;

constexpr U32 MAdd(U32 A, U32 B) {
    U32 res = A + B;
    return res < Mod ? res : res - Mod;
}

constexpr U32 MSub(U32 A, U32 B) {
    U32 res = A - B;
    return A < B ? res + Mod : res;
}

constexpr U32 MMul(U32 A, U32 B) {
    return (U32) ((U64) A * B % Mod);
}

constexpr U32 MPow(U32 A, U32 B) {
    U32 res = 1;
    while (B) {
        if (B & 1)
            res = MMul(res, A);
        A = MMul(A, A);
        B >>= 1;
    }
    return res;
}

inline_ __m256i VLod(const U32* restrict_ A) {
    return _mm256_load_si256((const __m256i*) A);
}

inline_ void VSto(U32* restrict_ A, __m256i v) {
    _mm256_store_si256((__m256i*) A, v);
}

inline_ __m256i VEx0(__m256i v) {
    const __m256i vm0 = _mm256_set_epi64x(
        0x111111111b1a1918, 0x1111111113121110,
        0x111111110b0a0908, 0x1111111103020100
    );
    return _mm256_shuffle_epi8(v, vm0);
}

inline_ __m256i VEx1(__m256i v) {
    const __m256i vm1 = _mm256_set_epi64x(
        0x111111111f1e1d1c, 0x1111111117161514,
        0x111111110f0e0d0c, 0x1111111107060504
    );
    return _mm256_shuffle_epi8(v, vm1);
}

inline_ __m256i VIntlv(__m256i v0, __m256i v1) {
    return _mm256_blend_epi32(v0, _mm256_shuffle_epi32(v1, 0xb1), 0xaa);
}

inline_ __m256i VAdd(__m256i va, __m256i vb) {
    const __m256i vm32 = _mm256_set1_epi32(Mod);
    __m256i vra = _mm256_add_epi32(va, vb);
    __m256i vrb = _mm256_sub_epi32(vra, vm32);
    return _mm256_min_epu32(vra, vrb);
}

inline_ __m256i VSub(__m256i va, __m256i vb) {
    const __m256i vm32 = _mm256_set1_epi32(Mod);
    __m256i vra = _mm256_sub_epi32(va, vb);
    __m256i vrb = _mm256_add_epi32(vra, vm32);
    return _mm256_min_epu32(vra, vrb);
}

inline_ __m256i VMul(__m256i va0, __m256i va1, __m256i vb0, __m256i vb1) {
    const __m256i vm32 = _mm256_set1_epi32(Mod);
    const __m256i vm64 = _mm256_set1_epi64x(Mod);
    const __m256i vbmm = _mm256_set1_epi64x(BmM);
    __m256i vmul0 = _mm256_mul_epi32(va0, vb0);
    __m256i vmul1 = _mm256_mul_epi32(va1, vb1);
    __m256i vlow = VIntlv(vmul0, vmul1);
    __m256i vquo0 = _mm256_srli_epi64(_mm256_mul_epi32(_mm256_srli_epi64(vmul0, 29), vbmm), BmW);
    __m256i vquo1 = _mm256_srli_epi64(_mm256_mul_epi32(_mm256_srli_epi64(vmul1, 29), vbmm), BmW);
    __m256i vval0 = _mm256_mul_epi32(vquo0, vm64);
    __m256i vval1 = _mm256_mul_epi32(vquo1, vm64);
    __m256i vval = VIntlv(vval0, vval1);
    __m256i vra = _mm256_sub_epi32(vlow, vval);
    __m256i vrb = _mm256_add_epi32(vra, vm32);
    __m256i vrc = _mm256_sub_epi32(vra, vm32);
    __m256i vmin = _mm256_min_epu32(vra, vrb);
    return _mm256_min_epu32(vmin, vrc);
}

inline_ __m256i VMul(__m256i va, __m256i vb0, __m256i vb1) {
    return VMul(VEx0(va), VEx1(va), vb0, vb1);
}

inline_ __m256i VMul(__m256i va, __m256i vb) {
    return VMul(va, VEx0(vb), VEx1(vb));
}

inline_ void VMul(U32* restrict_ A, U32 Len, U32 W) {
    if (Len < 8) {
        for (U32 i = 0; i < Len; ++i)
            A[i] = MMul(A[i], W);
        return;
    }
    __m256i vw = _mm256_set1_epi64x(W);
    for (U32 i = 0; i < Len; i += 8)
        VSto(A + i, VMul(VLod(A + i), vw, vw));
}

inline_ void VMul(U32* restrict_ A, const U32* restrict_ B, U32 Len) {
    if (Len < 8) {
        for (U32 i = 0; i < Len; ++i)
            A[i] = MMul(A[i], B[i]);
        return;
    }
    for (U32 i = 0; i < Len; i += 8)
        VSto(A + i, VMul(VLod(A + i), VLod(B + i)));
}

inline_ void VSqr(U32* restrict_ A, U32 Len) {
    if (Len < 8) {
        for (U32 i = 0; i < Len; ++i)
            A[i] = MMul(A[i], A[i]);
        return;
    }
    for (U32 i = 0; i < Len; i += 8) {
        __m256i va = VLod(A + i);
        __m256i v0 = VEx0(va);
        __m256i v1 = VEx1(va);
        VSto(A + i, VMul(v0, v1, v0, v1));
    }
}

constexpr U32 WbFwd = MPow(G, (Mod - 1) >> LogLen);
constexpr U32 WbInv = MPow(GInv, (Mod - 1) >> LogLen);
constexpr U32 LenInv = MPow(Len, Mod - 2);

inline_ void NttImpl1(U32* restrict_ A) {
    for (U32 j = 0; j < Len; j += 2) {
        U32 a0 = MAdd(A[j + 0], A[j + 1]);
        U32 b0 = MSub(A[j + 0], A[j + 1]);
        A[j + 0] = a0;
        A[j + 1] = b0;
    }
}

inline_ void NttFwd2(U32* restrict_ A, U32 Wn) {
    for (U32 j = 0; j < Len; j += 4) {
        U32 a0 = MAdd(A[j + 0], A[j + 2]);
        U32 a1 = MAdd(A[j + 1], A[j + 3]);
        U32 b0 = MSub(A[j + 0], A[j + 2]);
        U32 b1 = MSub(A[j + 1], A[j + 3]);
        A[j + 0] = a0;
        A[j + 1] = a1;
        A[j + 2] = b0;
        A[j + 3] = MMul(b1, Wn);
    }
}

inline_ void NttFwd3(U32* restrict_ A, U32 Wn) {
    U32 W2 = MMul(Wn, Wn);
    U32 W3 = MMul(W2, Wn);
    const __m128i vm32 = _mm_set1_epi32(Mod);
    for (U32 j = 0; j < Len; j += 8) {
        __m128i va = _mm_load_si128((const __m128i*) (A + j));
        __m128i vb = _mm_load_si128((const __m128i*) (A + j + 4));
        __m128i vc = _mm_add_epi32(va, vb);
        __m128i vd = _mm_sub_epi32(va, vb);
        __m128i ve = _mm_min_epu32(vc, _mm_sub_epi32(vc, vm32));
        __m128i vf = _mm_min_epu32(vd, _mm_add_epi32(vd, vm32));
        _mm_store_si128((__m128i*) (A + j), ve);
        _mm_store_si128((__m128i*) (A + j + 4), vf);
        A[j + 5] = MMul(Wn, A[j + 5]);
        A[j + 6] = MMul(W2, A[j + 6]);
        A[j + 7] = MMul(W3, A[j + 7]);
    }
}

inline_ void NttFwd(U32* restrict_ A) {
    U32 Wn = WbFwd;
    for (int i = LogLen - 1; i >= 3; --i) {
        U32 ChkSiz = 1u << i;
        U32 tw2 = MMul(Wn, Wn);
        U32 tw3 = MMul(tw2, Wn);
        U32 tw4 = MMul(tw3, Wn);
        U32 tw5 = MMul(tw4, Wn);
        U32 tw6 = MMul(tw5, Wn);
        U32 tw7 = MMul(tw6, Wn);
        U32 twn = MMul(tw7, Wn);
        __m256i vw32 = _mm256_set_epi32(tw7, tw6, tw5, tw4, tw3, tw2, Wn, 1);
        __m256i vwn = _mm256_set1_epi64x(twn);
        for (U32 j = 0; j < Len; j += 2u << i) {
            U32* A_ = A + j;
            U32* B_ = A_ + ChkSiz;
            __m256i vw = vw32;
            for (U32 k = 0; k < ChkSiz; k += 8) {
                __m256i va = VLod(A_ + k);
                __m256i vb = VLod(B_ + k);
                __m256i vw0 = VEx0(vw);
                __m256i vw1 = VEx1(vw);
                __m256i vc = VAdd(va, vb);
                __m256i vd = VSub(va, vb);
                VSto(A_ + k, vc);
                VSto(B_ + k, VMul(vd, vw0, vw1));
                vw = VMul(vw0, vw1, vwn, vwn);
            }
        }
        Wn = MMul(Wn, Wn);
    }
    if (LogLen >= 3) {
        NttFwd3(A, Wn);
        Wn = MMul(Wn, Wn);
    }
    if (LogLen >= 2)
        NttFwd2(A, Wn);
    if (LogLen)
        NttImpl1(A);
}

inline_ void NttInv2(U32* restrict_ A, U32 Wn) {
    for (U32 j = 0; j < Len; j += 4) {
        U32 a0 = A[j + 0];
        U32 a1 = A[j + 1];
        U32 b0 = A[j + 2];
        U32 b1 = MMul(A[j + 3], Wn);
        A[j + 0] = MAdd(a0, b0);
        A[j + 1] = MAdd(a1, b1);
        A[j + 2] = MSub(a0, b0);
        A[j + 3] = MSub(a1, b1);
    }
}

inline_ void NttInv3(U32* restrict_ A, U32 Wn) {
    U32 W2 = MMul(Wn, Wn);
    U32 W3 = MMul(W2, Wn);
    const __m128i vm32 = _mm_set1_epi32(Mod);
    for (U32 j = 0; j < Len; j += 8) {
        A[j + 5] = MMul(Wn, A[j + 5]);
        A[j + 6] = MMul(W2, A[j + 6]);
        A[j + 7] = MMul(W3, A[j + 7]);
        __m128i va = _mm_load_si128((const __m128i*) (A + j));
        __m128i vb = _mm_load_si128((const __m128i*) (A + j + 4));
        __m128i vc = _mm_add_epi32(va, vb);
        __m128i vd = _mm_sub_epi32(va, vb);
        __m128i ve = _mm_min_epu32(vc, _mm_sub_epi32(vc, vm32));
        __m128i vf = _mm_min_epu32(vd, _mm_add_epi32(vd, vm32));
        _mm_store_si128((__m128i*) (A + j), ve);
        _mm_store_si128((__m128i*) (A + j + 4), vf);
    }
}

inline_ void NttInv(U32* restrict_ A) {
    if (!LogLen)
        return;
    NttImpl1(A);
    if (LogLen == 1) {
        VMul(A, Len, LenInv);
        return;
    }
    U32 Ws[LogLen];
    Ws[0] = WbInv;
    for (int i = 1; i < LogLen; ++i)
        Ws[i] = MMul(Ws[i - 1], Ws[i - 1]);
    NttInv2(A, Ws[LogLen - 2]);
    if (LogLen == 2) {
        VMul(A, Len, LenInv);
        return;
    }
    NttInv3(A, Ws[LogLen - 3]);
    if (LogLen == 3) {
        VMul(A, Len, LenInv);
        return;
    }
    for (int i = 3; i < LogLen; ++i) {
        U32 ChkSiz = 1u << i;
        U32 Wn = Ws[LogLen - 1 - i];
        U32 tw2 = MMul(Wn, Wn);
        U32 tw3 = MMul(tw2, Wn);
        U32 tw4 = MMul(tw3, Wn);
        U32 tw5 = MMul(tw4, Wn);
        U32 tw6 = MMul(tw5, Wn);
        U32 tw7 = MMul(tw6, Wn);
        U32 twn = MMul(tw7, Wn);
        __m256i vw32 = _mm256_set_epi32(tw7, tw6, tw5, tw4, tw3, tw2, Wn, 1);
        __m256i vwn = _mm256_set1_epi64x(twn);
        for (U32 j = 0; j < Len; j += 2u << i) {
            U32* A_ = A + j;
            U32* B_ = A_ + ChkSiz;
            __m256i vw = vw32;
            for (U32 k = 0; k < ChkSiz; k += 8) {
                __m256i vw0 = VEx0(vw);
                __m256i vw1 = VEx1(vw);
                __m256i vb = VMul(VLod(B_ + k), vw0, vw1);
                vw = VMul(vw0, vw1, vwn, vwn);
                __m256i va = VLod(A_ + k);
                __m256i vc = VAdd(va, vb);
                __m256i vd = VSub(va, vb);
                VSto(A_ + k, vc);
                VSto(B_ + k, vd);
            }
        }
    }
    VMul(A, Len, LenInv);
}
}

constexpr int WU = 40;
constexpr int RU = 80;

alignas(32) uint32_t A[Len]{};
alignas(32) uint32_t B[Len]{};
alignas(32) uint32_t C[Len]{};
alignas(32) uint32_t Z[Len]{};

bool Equal(const uint32_t* restrict_ A, const uint32_t* restrict_ B) {
  return !memcmp(A, B, sizeof(uint32_t) * Len);
}

template<class F>
auto Test(F&& Trans, uint32_t* A) {
  for (int i = 0; i < WU; ++i) {
    Trans(A);
    MFence();
  }
  auto T0 = high_resolution_clock::now();
  for (int i = 0; i < RU; ++i) {
    Trans(A);
    MFence();
  }
  auto T1 = high_resolution_clock::now();
  return T1 - T0;
}

template<class F1, class F2>
void Pair(F1&& FA, F2&& FB, const char* S = "") {
  auto TA = duration_cast<microseconds>(Test(forward<F1>(FA), A));
  auto TB = duration_cast<microseconds>(Test(forward<F2>(FB), B));
  if (!Equal(A, B))
    cout << "Fuck: " << S << endl;
  else
    cout << "Done: " << S << endl;
  cout << "AVX2-" << S << ": " << TA.count() << " us" << endl;
  cout << "Norm-" << S << ": " << TB.count() << " us" << endl;
}

template<class F1, class F2, class F3>
void Trip(F1&& FA, F2&& FB, F3&& FC, const char* S = "") {
  auto TA = duration_cast<microseconds>(Test(forward<F1>(FA), A));
  auto TB = duration_cast<microseconds>(Test(forward<F2>(FB), B));
  auto TC = duration_cast<microseconds>(Test(forward<F3>(FC), C));
  if (!Equal(A, B) || !Equal(A, C))
    cout << "Fuck: " << S << endl;
  else
    cout << "Done: " << S << endl;
  cout << "AVX2-" << S << ": " << TA.count() << " us" << endl;
  cout << "Norm-" << S << ": " << TB.count() << " us" << endl;
  cout << "CSP5-" << S << ": " << TC.count() << " us" << endl;
}

int main() {
  using Avx2 = ntt::avx2::Ntt<Mod, G>;
  using Norm = ntt::norm::Ntt<Mod, G>;
  mt19937 MT{random_device{}()};
  uniform_int_distribution<uint32_t> U{0, Mod - 1};
  generate_n(Z, Len, bind(U, MT));
  copy_n(Z, Len, A);
  copy_n(Z, Len, B);
  copy_n(Z, Len, C);
  Trip(Avx2::FwdDif<LogLen>, Norm::FwdDif<LogLen>, csp5::NttFwd, "FwdDif");
  Trip(Avx2::InvDit<LogLen>, Norm::InvDit<LogLen>, csp5::NttInv, "InvDit");
  Pair(Avx2::FwdDit<LogLen>, Norm::FwdDit<LogLen>, "FwdDit");
  Pair(Avx2::InvDif<LogLen>, Norm::InvDif<LogLen>, "InvDif");
  if (Equal(A, Z))
    cout << "AVX2 Correct" << endl;
  else
    cout << "Fuck AVX2" << endl;
  if (Equal(B, Z))
    cout << "Norm Correct" << endl;
  else
    cout << "Fuck Norm" << endl;
  if (Equal(C, Z))
    cout << "CSP5 Correct" << endl;
  else
    cout << "Fuck CSP5" << endl;
  return 0;
}
