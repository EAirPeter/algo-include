...
public:
  ...
  static inline_ void FwdDif(uint32_t* restrict_ A) noexcept {
    X_Dif<LogLen, G>(A, Exps{});
  }

  template<int LogLen, class Exps = MkIntSeq<LogLen>>
  static inline_ void FwdDit(uint32_t* restrict_ A) noexcept {
    X_Dit<LogLen, G>(A, Exps{});
  }

  template<int LogLen, class Exps = MkIntSeq<LogLen>>
  static inline_ void InvDif(uint32_t* restrict_ A) noexcept {
    X_Dif<LogLen, X_InvG>(A, Exps{});
    if constexpr (LogLen > 0) {
      constexpr auto InvLen = Inv((uint32_t) ((size_t{1} << LogLen) % Mod));
      VMul<LogLen>(A, InvLen);
    }
  }

  template<int LogLen, class Exps = MkIntSeq<LogLen>>
  static inline_ void InvDit(uint32_t* restrict_ A) noexcept {
    X_Dit<LogLen, X_InvG>(A, Exps{});
    if constexpr (LogLen > 0) {
      constexpr auto InvLen = Inv((uint32_t) ((size_t{1} << LogLen) % Mod));
      VMul<LogLen>(A, InvLen);
    }
...
private:
  ...
  template<int LogLen, uint32_t R, int... Exps>
  static void X_Dif(uint32_t* restrict_ A, IntSeq<Exps...>) noexcept {
    if constexpr (LogLen > 0) {
      (..., X_DifAt<
        LogLen,
        LogLen - Exps - 1,
        Pow(R, (Mod - 1) >> (LogLen - Exps))
      >(A));
    }
  }

  template<int LogLen, uint32_t R, int... Exps>
  static void X_Dit(uint32_t* restrict_ A, IntSeq<Exps...>) noexcept {
    if constexpr (LogLen > 0) {
      (..., X_DitAt<
        LogLen,
        Exps,
...
private:
  ...
  static inline_ void X_DifAt(uint32_t* restrict_ A) noexcept {
    constexpr auto Len = size_t{1} << LogLen;
    if constexpr (Exp <= 0) {
      if constexpr (LogLen <= 0) {}
      else if constexpr (LogLen == 1) {
  ...
  template<int LogLen, int Exp, uint32_t Wn>
  static inline_ void X_DitAt(uint32_t* restrict_ A) noexcept {
    constexpr auto Len = size_t{1} << LogLen;
    if constexpr (Exp <= 0) {
      if constexpr (LogLen <= 0) {}
      else if constexpr (LogLen == 1) {
      ...
      if constexpr (LogLen <= 1) {}
        ...
        const auto vwn = _mm256_set1_epi64x(Wn);
        for (size_t i = 0; i < Len; i += 16) {
          auto vr0 = X_VLodY(A + i + 0);
          auto vr1 = X_VLodY(A + i + 8);
          auto vy0 = _mm256_shuffle_epi32(vr0, 0xff); // 0q3333
          auto vy1 = _mm256_shuffle_epi32(vr1, 0xff); // 0q3333
          auto vy = _mm256_blend_epi32(vy0, vy1, 0xcc); // 0b11001100
          auto vz = X_VMulYp(vy, vwn);
          auto vz0 = _mm256_shuffle_epi32(vz, 0x00); // 0q0000
          auto vz1 = _mm256_shuffle_epi32(vz, 0xaa); // 0q2222
          auto va0 = _mm256_blend_epi32(vr0, vz0, 0x88); // 0b10001000
          auto va1 = _mm256_blend_epi32(vr1, vz1, 0x88); // 0b10001000
          auto vb0 = _mm256_shuffle_epi32(va0, 0x4e); // 0q1032
          auto vb1 = _mm256_shuffle_epi32(va1, 0x4e); // 0q1032
          auto vc00 = _mm256_add_epi32(va0, vb0);
          auto vc01 = _mm256_sub_epi32(vb0, va0);
      ...
      if constexpr (LogLen <= 2) {}
        ...
        auto vr = X_VLodX(A + 4);
        auto va = X_VLodX(A + 0);
        auto vb = X_VMulX(vr, vw);
        auto vc = X_VAddX(va, vb);
        auto vd = X_VSubX(va, vb);
        X_VStoX(A + 0, vc);
        X_VStoX(A + 4, vd);
      }
      else if constexpr (LogLen < X_Ubll) {
        ...
        for (size_t i = 0; i < Len; i += 16) {
          ...
          auto va_ = _mm256_loadu_si256(
            (const __m256i* restrict_) (A + i + 4));
          auto va1 = X_VLodY(A + i + 8);
          auto vr = _mm256_blend_epi32(va_, va1, 0xf0); // 0b11110000
          auto va = _mm256_blend_epi32(va0, va_, 0xf0); // 0b11110000
          auto vb = X_VMulY(vr, vw0, vw1);
          auto vc0 = X_VAddY(va, vb);
          auto vd1 = X_VSubY(va, vb);
          auto vc1 = _mm256_permute4x64_epi64(vc0, 0xee); // 0q3232
          auto vd0 = _mm256_permute4x64_epi64(vd1, 0x44); // 0q1010
          auto vx1 = _mm256_blend_epi32(vc1, vd1, 0xf0); // 0b11110000
          auto vx0 = _mm256_blend_epi32(vc0, vd0, 0xf0); // 0b11110000
          X_VStoY(A + i + 8, vx1);
          X_VStoY(A + i + 0, vx0);
        }
      }
      ...
      if constexpr (LogLen <= 3) {}
        ...
        auto va = X_VLodY(A + 0);
        auto vb = X_VMulY(vr, vw0, vw1);
        auto vc = X_VAddY(va, vb);
        auto vd = X_VSubY(va, vb);
        X_VStoY(A + 0, vc);
        X_VStoY(A + 8, vd);
      }
      else if constexpr (LogLen < X_Ubll) {
  ...
  static inline_ __m128i X_VMulX(__m256i va, __m256i vb) noexcept {
    ...
    auto vd = _mm256_srli_epi64(vc, X_MF);
    auto vr = _mm256_srli_epi64(_mm256_mul_epu32(vd, x_vhy64), X_MG);
    ...
    __m256i va0, __m256i va1, __m256i vb0, __m256i vb1) noexcept
  {
    ...
    auto vd0 = _mm256_srli_epi64(vc0, X_MF);
    auto vd1 = _mm256_srli_epi64(vc1, X_MF);
    auto vr0 = _mm256_srli_epi64(_mm256_mul_epu32(vd0, x_vhy64), X_MG);
    auto vr1 = _mm256_srli_epi64(_mm256_mul_epu32(vd1, x_vhy64), X_MG);
    auto vs0 = _mm256_mul_epu32(vr0, x_vmy64);
    auto vs1 = _mm256_mul_epu32(vr1, x_vmy64);
    auto vc1_ = _mm256_shuffle_epi32(vc1, 0xa0); // 0q2200
    auto vs1_ = _mm256_shuffle_epi32(vs1, 0xa0); // 0q2200
    auto vc = _mm256_blend_epi32(vc0, vc1_, 0xaa); // 0b10101010
    auto vs = _mm256_blend_epi32(vs0, vs1_, 0xaa); // 0b10101010
  ...
  static constexpr int X_GetMF() noexcept {
    int MF = 0;
    while (MF < 32 && (1u << (MF + 1)) <= Mod)
      ++MF;
    return MF;
  }

  static constexpr int X_GetMG(int MF) noexcept {
    auto M2 = (uint64_t) (Mod - 1) * (Mod - 1);
    int MG = 0;
    while (MF + MG < 64 && (1ull << (MF + MG)) < M2)
      ++MG;
    return MG;
  }

  static constexpr uint32_t X_GetMH(int MF, int MG) noexcept {
    return (uint32_t) ((uint64_t{1} << (MF + MG)) / Mod);
  }

  static constexpr int X_Ubll = ::std::numeric_limits<size_t>::digits;
  static constexpr int X_MF = X_GetMF();
  static constexpr int X_MG = X_GetMG(X_MF);
  static constexpr int X_MH = X_GetMH(X_MF, X_MG);
  ...
  static inline const auto x_vhy64 = _mm256_set1_epi64x(X_MH);
  static inline const auto x_vpy = _mm256_set_epi32(7, 5, 3, 1, 6, 4, 2, 0);

#else // !defined(NTT_USE_AVX2)
public:
  template<int LogLen>
  static constexpr void VAdd(uint32_t* restrict_ A,
    const uint32_t* restrict_ B) noexcept
  {
    constexpr auto Len = size_t{1} << LogLen;
  ...
  template<int LogLen>
  static constexpr void VAdd(uint32_t* restrict_ A, uint32_t B) noexcept {
    constexpr auto Len = size_t{1} << LogLen;
    for (size_t i = 0; i < Len; ++i)
      A[i] = Add(A[i], B);
  }
  ...
  template<int LogLen>
  static constexpr void VAdd(uint32_t* restrict_ A) noexcept {
    constexpr auto Len = size_t{1} << LogLen;
    for (size_t i = 0; i < Len; ++i)
      A[i] = Add(A[i], A[i]);
  }
  ...
  template<int LogLen>
  static constexpr void VSub(uint32_t* restrict_ A,
    const uint32_t* restrict_ B) noexcept
  {
    constexpr auto Len = size_t{1} << LogLen;
    for (size_t i = 0; i < Len; ++i)
  ...
  template<int LogLen>
  static constexpr void VSub(uint32_t* restrict_ A, uint32_t B) noexcept {
    constexpr auto Len = size_t{1} << LogLen;
    for (size_t i = 0; i < Len; ++i)
      A[i] = Sub(A[i], B);
  }
  ...
  template<int LogLen>
  static constexpr void VMul(uint32_t* restrict_ A,
    const uint32_t* restrict_ B) noexcept
  {
    constexpr auto Len = size_t{1} << LogLen;
    for (size_t i = 0; i < Len; ++i)
  ...
  template<int LogLen>
  static constexpr void VMul(uint32_t* restrict_ A, uint32_t B) noexcept {
    constexpr auto Len = size_t{1} << LogLen;
    for (size_t i = 0; i < Len; ++i)
      A[i] = Mul(A[i], B);
  }
  ...
  template<int LogLen>
  static constexpr void VMul(uint32_t* restrict_ A) noexcept {
    constexpr auto Len = size_t{1} << LogLen;
    for (size_t i = 0; i < Len; ++i)
      A[i] = Mul(A[i], A[i]);
  }
...
private:
  template<int LogLen, int Exp, uint32_t Wn>
  static constexpr void X_DifAt(uint32_t* restrict_ A) noexcept {
    constexpr auto Len = size_t{1} << LogLen;
    constexpr auto Leap = size_t{2} << Exp;
    constexpr auto Step = size_t{1} << Exp;
    for (size_t i = 0; i < Len; i += Leap) {
  ...
  template<int LogLen, int Exp, uint32_t Wn>
  static constexpr void X_DitAt(uint32_t* restrict_ A) noexcept {
    constexpr auto Len = size_t{1} << LogLen;
    constexpr auto Leap = size_t{2} << Exp;
    constexpr auto Step = size_t{1} << Exp;
    for (size_t i = 0; i < Len; i += Leap) {
...
#endif // ifdef NTT_USE_AVX2

};

}} // namespace ntt::{norm|avx2}
